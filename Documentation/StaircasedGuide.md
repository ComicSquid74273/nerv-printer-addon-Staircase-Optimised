...
Before entering any U or independent fallback line, the bot checks the actual remaining durability of every compatible required tool. The check assumes the worst case of one durability per block, applies the configured durability buffer, ignores the probabilistic benefit of Unbreaking, and preloads enough fresh tools to finish the assigned route. The teardown loadout has an additional hard minimum of two registered pickaxes and one registered axe. A compatible worn tool already carried contributes its remaining usable durability, but a worn tool in a source chest cannot satisfy a demand for a fresh replacement. Empty, incompatible, worn-only, or insufficient tool chests stop the bot before it enters the map. Teardown also reserves **teardown-scaffold-stacks** of cobblestone (`3` by default, configurable from `2` to `3`). Partial carried stacks count toward that exact reserve and their free stack capacity is used before the capacity planner requests another inventory slot. A registered cobblestone chest and enough managed capacity are required before the bot enters teardown, so sparse recovery never begins by hoping that material remains.
