...
Before a build refill, reusable tools reserve their managed slots and are never sent to the material DumpStation. A carried tool is replaced only when it has 20 durability or less. That exact slot is deposited into its matching single used-tool chest; the initially selected Used Pickaxe Chest is the fallback. Deposits are slot-specific, so a critical and a healthy tool of the same type are not moved together. The printing carry plan requests only one pickaxe and one axe, unless a wrong active-U block requires another registered tool type. After mining, all remaining registered tools are returned to used-tool storage.
...
Before entering any U or independent fallback line, the bot checks the actual remaining durability of every compatible required tool. The check assumes the worst case of one durability per block, applies the configured durability buffer, ignores the probabilistic benefit of Unbreaking, and preloads enough fresh tools to finish the assigned route. The teardown loadout has an additional hard minimum of two registered pickaxes and one registered axe. A compatible worn tool already carried contributes its remaining usable durability, but a worn tool in a source chest cannot satisfy a demand for a fresh replacement. Empty, incompatible, worn-only, or insufficient tool chests stop the bot before it enters the map.

After the strict tool restock is server-confirmed, the entry checkpoint assigns the two pickaxes and axe to three concrete hotbar slots and silently fills missing assignments with the same acknowledged packet-swap controller used during printing. Existing correct tools remain in place. During teardown, the block-specific tool selector uses only a compatible tool already assigned in the hotbar; when an assigned tool is exhausted, its replacement may enter only that same planned item slot. There is no arbitrary first-slot eviction path.

Ordered teardown uses the same **max-block-actions-per-second** TPS-scaled ceiling as printing, but it never adds an unordered spatial breaking scan. At every safe assignment boundary it proves monotonic reach schedules from each candidate walking U to every target in later U routes. A route may therefore tear down the next and next-next U, or any other fully scheduled U inside the centered five-block reach proof, and those completed routes are skipped. The proof retains 0.20 blocks of positional margin; reach-assigned actions are executed only while grounded at the center checkpoint and still pass the live range check. A partially completed remote route remains an ordered prefix/suffix recovery shape rather than arbitrary missing blocks.
...
Mixed-height teardown movement retains the ordered segment direction after crossing a support center, so gravity can finish a one-block descent without a 180-degree correction toward the point just crossed. Auto-jump is evaluated against that exact ordered support rather than whichever obstacle happens to be in the current yaw direction; it cannot begin the next stair jump before the current checkpoint is consumed and then have the break transition cancel it. A true-instant local break behind the bot may await its authoritative air acknowledgement while movement continues toward the next verified support. Progressive mining and reach-assigned breaks retain their movement barrier because moving could abandon the owned mine or leave the proved reach window. If a grounded route step makes no meaningful progress for two seconds, the bot logs the player, previous support, target support, and queue size, then reconstructs the remaining path from the authoritative U support under the player. It does not remain indefinitely in `MiningUTraversal` with no diagnostic.
